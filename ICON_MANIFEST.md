...
**RECOMMENDATIONS.md item 88 — ships as a deliberate design library, not dead weight.** Of the 155 icons, roughly a dozen are wired to a live nav item or UI surface today; the rest are marked `unmapped — spare inventory` in the table below, one row per icon, not lumped into a vague "unused" bucket. They are kept rather than pruned because every icon shares the same four source sheets, art style and processing pipeline (see (a)–(d) below), so a future surface (`/managers`, `/venues`, settings sub-pages, admin tooling, etc.) can pull a matching icon on demand instead of commissioning or re-processing new art. Each is a natural-resolution WebP (tens of KB, not a shared spritesheet), so the unused ones cost real but small deployed bytes — acceptable for a launch-stage app where new surfaces ship often and icon art is otherwise a bottleneck. Revisit this if the unused set grows much further without new surfaces consuming it.

## (a) Alpha transparency finding

- **The four icon sheets have NO real alpha channel** (sharp reports `channels: 3, hasAlpha: false` for all of them; corner pixels are near-pure black, e.g. `RGB(1,1,8)`, `RGB(0,0,0)`). The original slicing pass correctly avoided naive color-keying (near-black-to-transparent), since large parts of several icons' own artwork are also near-black and directly touch the sheet background (phone bezels, scoreboard faces, VAR-screen frames) — a flat color key would have eaten real content, confirmed by testing it directly (see below).
- **Fixed in a follow-up pass**: instead of color-keying, every one of the 155 icons was run through `rembg`'s u2net segmentation model (a trained foreground/background classifier, not a color threshold), which correctly separates each icon's floating 3D object from its background even where both are the same near-black color, because it reasons about object boundaries rather than pixel color. Verified by testing the flat color-key approach first (it did visibly eat into bezel/screen detail, confirming the original caution was correct) and then testing u2net (clean cutouts, no fringing, spot-checked across a random 8-icon sample spanning all four source sheets and multiple categories). All 155 files were reprocessed in place as lossless WebP with real alpha.
- **The logo is the one asset this fix does NOT apply to.** Its original alpha channel was a soft, semi-transparent gradient baked in at generation time (not a flat background), and running the same u2net model against it reproduced the same blotchy fringe around the wordmark/tagline — the model struggles with the logo's own soft edges rather than solving them. The logo stays **flattened onto solid KIVO Obsidian (`#05060A`)**, per the original finding below.
