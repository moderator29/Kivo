        ...
        {rows.map((row, rowIndex) => (
          <div key={row.key} className="flex flex-wrap items-start justify-center gap-3">
            {row.players.map((p, i) => {
              const token = (
                <>
                  <span className="flex h-9 w-9 items-center justify-center rounded-full border border-hairline bg-surface-1 text-xs font-semibold text-foreground transition group-hover:bg-surface-2 group-hover:text-accent">
