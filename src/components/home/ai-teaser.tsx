...
export function AiTeaser({ aiConfigured }: { aiConfigured: boolean }) {
  return (
    <Link
      href="/ai"
      className="kivo-gradient-intelligence group relative flex items-center gap-3 overflow-hidden rounded-2xl p-4 transition-opacity hover:opacity-90"
    >
      <motion.span
        aria-hidden
      ...
      >
        <Sparkles className="h-5 w-5 text-kivo-white" strokeWidth={1.75} />
      </motion.span>
      <p className="relative z-10 flex-1 text-sm font-medium text-kivo-white">
        {aiConfigured
      ...
      <ArrowRight className="relative z-10 h-4 w-4 shrink-0 text-kivo-white/80 transition-transform group-hover:translate-x-0.5" />
    </Link>
  );
}
