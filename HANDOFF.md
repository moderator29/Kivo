...
**The things only the founder can do, documented in `ENVIRONMENT.md`**:
...
2. Before real signups: configure custom SMTP in the Supabase dashboard. Supabase's built-in sender delivers the sign-in code but is rate-limited to a handful of messages per hour per project, which is fine for testing and not for launch.

**What used to be item 2 on this list is gone.** The Supabase↔Clerk third-party-auth dashboard step — a founder-only, code-unreachable click without which *every* RLS-gated query was rejected no matter what env vars were set — no longer exists, because Supabase now issues the JWTs it verifies. A fresh checkout goes from three pasted Supabase keys to a working, signed-in app with nothing to configure by hand in either dashboard. The SMTP item above is a real but much softer prerequisite: it affects delivery volume, not whether auth works.
