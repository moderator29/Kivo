...
import noBareConsoleError from "./eslint-rules/no-bare-console-error.mjs";
    ...
    plugins: {
      kivo: {
        rules: {
          ...
          "no-bare-console-error": noBareConsoleError,
        },
      },
    },
    rules: {
      // KN-129: a structured error sink only some failures reach is worse than
      // none, because an empty log search reads as "no errors".
      "kivo/no-bare-console-error": "error",
    },
  },
  {
    files: ["src/**/*.tsx"],
