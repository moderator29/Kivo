...
### 2026-08-19 — `data_conflicts` is not built: it already exists as `data_anomalies`

**Decision**: The founding brief names three tables KIVO does not have — `data_conflicts`, `team_aliases`, `player_aliases`. The two alias tables were built (migration 0090). `data_conflicts` was deliberately **not** built, because it is the table `data_anomalies` (migration 0056) already is.

**What the audit found.** `data_anomalies`'s own column comment reads "Persisted record of a data conflict the pipeline detected". It carries a `data_anomaly_type` enum whose values are `score_regression`, `status_regression`, `duplicate_event`, `absent_entity` and `provider_disagreement`; the sync run that observed it; the provider; the entity, both by provider id and by KIVO id; `previous_value` and `new_value` as jsonb, so a row always names the two values that disagreed rather than summarising them in prose; and a review workflow with an RLS policy that stops an admin clearing a review or attributing it to somebody else. A `data_conflicts` table would have every one of those columns.

**Why building it anyway would have been worse than not building it.** Two tables holding one concept means the admin queue has two lists to read, two "unreviewed" counts, and a permanent question about which one a given detection belongs in. The name in the brief is a name, not a second requirement.

**What the audit did find, and it is the part that mattered.** `provider_disagreement` had a value in the enum, a label in the admin panel's anomaly summary, and **no producer anywhere in the codebase**. The one anomaly type the brief names most explicitly was the one KIVO could not report. That is now written by `upsertFixture` (`src/lib/football/sync.ts`), on the distinction that makes it meaningful: a value changing under the *same* provider is that provider revising itself, which the existing regression checks already cover; a value differing from what a *different* provider wrote is two sources disagreeing about a fact, which is a different problem with a different fix. Detection is one batched `provider_mappings` query per sync run, not one per fixture.

**Stated plainly**: with one provider enabled, that producer never fires. KIVO runs a single provider at a time (see the provider-failover entry), so today the map of "fixtures another provider has also written" is empty on every run. It stops being empty the moment a second provider is switched on — which is exactly when somebody needs the detection and exactly the wrong time to start building it.

**Why the alias tables were built rather than deferred.** Provider ids are how KIVO identifies an entity, and that is correct — names are not identifiers. But names are how *people* identify one, and they vary: "Man Utd" and "Manchester United", a club that renamed, and the losing name every single time an admin merges two duplicate rows. Every football sync overwrites `teams.name` with whatever the provider sent, which is right and which silently destroyed the previous name. Two automatic producers now keep them: an `AFTER INSERT` trigger on `entity_merges` records a merged row's name against the survivor, and an `AFTER UPDATE` trigger on `teams`/`players` records a name at the moment it is replaced. `resolve_football_entities` matches aliases alongside real names and still returns the entity's canonical name as the label.

**No alias is ever inferred.** Every row carries a `source` — `provider`, `merge`, `admin` or `rename` — saying where the name came from. `rename` exists because a trigger genuinely cannot tell whether a sync or an admin caused the change, and putting a false provenance on a real row would be worse than an honest one. Fuzzy matching is how aliases are *used*, never how they are created.

**Not globally unique, on purpose.** Uniqueness is per entity. "Arsenal" is a real name for more than one football club, and a global constraint would make recording the second one an error rather than the true statement it is. Ambiguity is handled where aliases are read, by returning both and letting the caller see the collision, rather than by silently picking one.
